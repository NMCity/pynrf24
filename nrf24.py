...
class NRF24:
	...
	def print_status(self, status):
		str = "STATUS\t = 0x%02x RX_DR=%x TX_DS=%x MAX_RT=%x RX_P_NO=%x TX_FULL=%x\r\n" % (\
           		status, \
	...
	def print_address_register(self, name, reg, qty = 1):
		...
		print ("%s\t%c =") % (name,extra_tab),
  		...
  		while (qty > 0):
			qty -= 1
    			buffer = reversed(self.read_register(reg, 5))
			reg += 1
    			sys.stdout.write(" 0x"),
			for i in buffer:
	...
	def write(self, buf):
		...
		while True:
			...
			if ( status & ( _BV(NRF24.TX_DS) | _BV(NRF24.MAX_RT) ) ) or ( int(round(time.time()*1000) - sent_at > timeout ) ):
				break
	...
	def openReadingPipe(self, child, address):
		# If this is pipe 0, cache the address.  This is needed because
  		...
  		if child <= 6:
    			...
    			self.write_register(NRF24.EN_RXADDR,self.read_register(NRF24.EN_RXADDR) | _BV(NRF24.child_pipe_enable[child]))

	def toogle_features(self):
  		buffer = [ NRF24.ACTIVATE, 0x73 ]
	...
	def enableDynamicPayloads(self):
		...
		self.write_register(NRF24.FEATURE,self.read_register(NRF24.FEATURE) | _BV(NRF24.EN_DPL) )
  		...
  		self.write_register(NRF24.DYNPD,self.read_register(NRF24.DYNPD) | _BV(NRF24.DPL_P5) | _BV(NRF24.DPL_P4) | _BV(NRF24.DPL_P3) | _BV(NRF24.DPL_P2) | _BV(NRF24.DPL_P1) | _BV(NRF24.DPL_P0))
	...
	def enableAckPayload(self):
  		...
  		self.write_register(NRF24.FEATURE,self.read_register(NRF24.FEATURE) | _BV(NRF24.EN_ACK_PAY) | _BV(NRF24.EN_DPL) )
